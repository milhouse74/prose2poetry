...
The pronouncingpy library includes rhyming word lookups. The implementation extracts the rhyming part of the phonemes of the target word and returns other words with the same rhyming part. There is an example in table \ref{table:pronouncingrhymes}.
...
It was noted that \citet{cole} used this function to extract rhyming pairs of sentences from the Gutenberg poetry corpora to prepare input couplets. The same function will be used to generate rhyming word pairs in \textit{prose2poetry}, and also for filtering the baseline corpora in section \ref{sec:corpora}.
...
\begin{tight_enumerate}
	...
	\item \textit{Word-pair scoring}: Given a list of all the possible word pairs, each pair can be scored with their respective semantic distance to the theme, with a multiplier of 1 or 0 if the words rhyme with each other. The top-scoring pairs in the final list are used as inputs to the sentence generation.
\end{tight_enumerate}
...
The filtering process uses the rhymes function available in the pronouncingpy library to extract every pair with rhyming end words from the same poem as a couplet.
...
A major component of the couplet score is the rhyme score. From the pronouncingpy rhymes function used to generate rhyming word pairs in the \textit{prose2poetry} model, the only information available is binary -- do the words rhyme or not. For an analysis of couplet quality, a more sophisticated score is needed that considers other possibilities, such as words that may not match exactly but have some other form of interior rhyme.
...
\begin{figure}
\begin{Verbatim}[fontsize=\small]
>>> from prose2poetry import rhyme_score
...
\begin{tight_itemize}
	\vspace{-0.5em}
	\item \textit{Rhyme score}: Described in \ref{sec:rhymescore}.
...
\begin{tabular}{lll c c c}
	\hline\hline
	Stress & Rhyme & Doc2vec & METEOR \\ [0.5ex]
	\hline\hline
	50\% & 20\% & 15\% & 15\% \\ [0.5ex]
	\hline
\end{tabular}
...
\hline\hline
\multicolumn{1}{|c|}{\phantom{Metric}} & \multicolumn{3}{c|}{Couplet} & \multicolumn{3}{c|}{Stress} & \multicolumn{3}{c|}{Doc2vec} & \multicolumn{3}{c|}{METEOR}\\
\cline{1-13}
\multicolumn{1}{|c|}{Dataset $\downarrow$} & $\bar{x}$ & SD & .95q & $\bar{x}$ & SD & .95q & $\bar{x}$ & SD & .95q & $\bar{x}$ & SD & .95q \\
\hline\hline
\textbf{Model A} & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 \\ [0.5ex]
\hline
\textbf{Model B} & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 & 0.5 \\ [0.5ex]
\hline
